...
void Monochrome_Palette_Window::initial_setup() {
	...
	_window = new Swatch_Window(_dx, _dy, 107+phlw, 192, "Edit Current Palettes");
	...
	int rgblw = std::max(std::max(text_width("R:", 2), text_width("G:", 2)), text_width("B:", 2));
	int rgbsw = text_width("99", 2) + 22;
	_red_spinner = new Default_Spinner(10+rgblw, 44, rgbsw, 22, "R:");
	_red_slider = new Default_Slider(16+rgblw+rgbsw, 44, 81+phlw-rgblw-rgbsw, 22);
	_green_spinner = new Default_Spinner(10+rgblw, 70, rgbsw, 22, "G:");
	_green_slider = new Default_Slider(16+rgblw+rgbsw, 70, 81+phlw-rgblw-rgbsw, 22);
	_blue_spinner = new Default_Spinner(10+rgblw, 96, rgbsw, 22, "B:");
	_blue_slider = new Default_Slider(16+rgblw+rgbsw, 96, 81+phlw-rgblw-rgbsw, 22);
	int hco = 10 + text_width("Hex: #", 2);
	int hcw = std::max(text_width("AAAAAA", 2), text_width("FFFFFF", 2));
	_hex_color_rgb = new OS_Hex_Input(hco, 122, hcw, 22, "Hex: #");
	_hex_color_swatch = new Fl_Button(hco+hcw+4, 122, 22, 22);
#ifdef _WIN32
	_ok_button = new Default_Button(-77+phlw, 160, 80, 22, "OK");
	_cancel_button = new OS_Button(17+phlw, 160, 80, 22, "Cancel");
#else
	_cancel_button = new OS_Button(-77+phlw, 160, 80, 22, "Cancel");
	_ok_button = new Default_Button(17+phlw, 160, 80, 22, "OK");
#endif
	_window->end();
	// Populate swatch group
	_color_group->color(FL_INACTIVE_COLOR);
